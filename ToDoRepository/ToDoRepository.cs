...
namespace ToDoRepository
{
    public class ToDoRepository : IToDoRepository
    {
        private static readonly List<TodoItem> _todoItems = new List<TodoItem> {
            ...
            new TodoItem(2, "Do staf", false, DateTime.UtcNow, DateTime.UtcNow, 1),
            new TodoItem(3, "Lable 3", true, DateTime.UtcNow, DateTime.UtcNow, 11),
            new TodoItem(4, "Lable 4", false, DateTime.UtcNow, DateTime.UtcNow, 2),
            new TodoItem(5, "Lable 5", false, DateTime.UtcNow, DateTime.UtcNow, 3)
        };
        private readonly IUserRepository _users;
        public ToDoRepository(IUserRepository users) {  _users = users; }
        ...
        public IEnumerable<TodoItem> GetList(int? offset, string? lable, int? ownerId, int? limit = 10)
        {
            ...
            if(ownerId.HasValue)
            {
                todos = todos.Where(i => i.OwnerId == ownerId.Value);
            }
        ...
        {
            var user = _users.GetById(item.OwnerId);
            if (user != null)
            {
                if (item != null)
                {
        ...
        public TodoItem? Put(int id, TodoItem newItem)
        {
            ...
            {
                var user = _users.GetById(newItem.OwnerId);
                if (user != null)
                {
